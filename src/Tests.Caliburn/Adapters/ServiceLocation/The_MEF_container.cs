namespace Tests.Caliburn.Adapters.ServiceLocation
{
	...
	public class The_MEF_container : ServiceLocatorTests
	{
		...
		[Test]
		public void Can_use_instance_registering_using_adapter()
		{
			var catalog = new AssemblyCatalog(Assembly.GetAssembly(typeof(ClassWithBehaviour)));
			var container = new CompositionContainer(catalog);
			...
			var comp = new Components.Component();

			adapter.Register(new IComponentRegistration[]
			                 	{
			                 		new Instance()
			                 			{
			                 				Service = typeof (Components.Component),
			                 				Implementation = comp
			                 			}
			                 	});

			var resolved = adapter.GetInstance<Components.Component>();
			...
			var user = adapter.GetInstance<User>();
			Assert.AreSame(user.Comp, comp);
		}

		[Test]
		public void Can_use_instance_registering_using_adapter_and_proxying()
		{
			var catalog = new AssemblyCatalog(Assembly.GetAssembly(typeof(ClassWithBehaviour)));
			var proxycatalog = new ProxyCatalog(catalog);
			var container = new CompositionContainer(proxycatalog);
